  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[35rem]">
        <DialogHeader>
          <DialogTitle className="pb-4">{postId ? 'Edit Post' : 'Create Post'}</DialogTitle>
          <SelectCustom
        ...
        <DialogFooter>
          <DialogTrigger asChild>
            <Button ref={btnCloseRef} className="w-[5rem] px-[3rem]" variant={'outline'}>
              Cancel
            </Button>
          </DialogTrigger>

          <Button className="w-[5rem] px-[3rem] bg-base-6" onClick={handlePost} disabled={loading}>
            Post
          </Button>
        </DialogFooter>
      </DialogContent>
